---
title: "KW second pass at Taves data"
date: 2018-12-10
...
```{r, include = F}
VSS(d_base_num, cor = "poly", rotate = "oblimin", fm = "wls")
```
...
```{r, include = F}
reten_fun(d_base_num, rot_type = "oblimin", fm_type = "wls")
```

## Factor loadings

```{r}
loadings <- fa(d_base_num, cor = "poly", nfactors = 8, rotate = "oblimin", fm = "wls")$loadings[] %>% 
  fa.sort() %>%
  data.frame() %>%
...
```{r}
scores <- fa(d_base_num, cor = "poly", nfactors = 8, rotate = "oblimin", fm = "wls")$scores[] %>% 
  data.frame() %>%
  rownames_to_column("taves_subj") %>%
...
```{r}
averages <- d_responses_named %>%
  group_by(factor, taves_ctry, taves_subj) %>%
  mutate(response_num = recode(response,
                               "no" = 0, "yes" = 1)) %>%
  summarise(mean = mean(response_num, na.rm = T)) %>%
  ungroup() %>%
  mutate(taves_ctry = factor(taves_ctry,
                             levels = c("US", "Ghana", "Thailand", 
                                        "China", "Vanuatu")),
...
averages %>%
  group_by(factor, taves_ctry) %>%
  langcog::multi_boot_standard(col = "mean", na.rm = T) %>%
  ungroup() %>% 
  mutate(factor = factor(factor,
                         levels = c("WLS2", "WLS4", "WLS5", "WLS1",
                                    "WLS6", "WLS3", "WLS7", "WLS8"))) %>%
...
```{r, fig.width = 6, fig.asp = 0.68, include = F}
# hkmeans8 %>% fviz_cluster(palette = "Dark2")
```
