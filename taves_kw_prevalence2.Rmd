...
df_appraisal <- d_appraisal_num2 %>%
  ...
  group_by(taves_ctry, level1, level2uncommon, level2common, level3, 
           cluster, question) %>%
  summarise(app = mean(appraisal), app_n = n()) %>%
  ungroup()
...
df_valence <- d_valence_num2 %>%
  ...
  group_by(taves_ctry, level1, level2uncommon, level2common, level3, 
           cluster, question) %>%
  summarise(val = mean(valence), val_n = n()) %>%
  ungroup()
...
df_significance <- d_significance_num2 %>%
  ...
  group_by(taves_ctry, level1, level2uncommon, level2common, level3, 
           cluster, question) %>%
  summarise(sig = mean(significance), sig_n = n()) %>%
  ungroup()

df_all_resp <- hclust_df_grouped %>% 
  rename(prev_US = US_prev, prev_n = n) %>%
  ...
  select(taves_ctry, level1, level2, level2uncommon, level2common, level3, 
         ...
         prev, prev_n, app, app_n, val, val_n, sig, sig_n)
```

```{r, fig.width = 6, fig.asp = 0.8}
df_all_resp %>%
  ggplot(aes(x = question, y = prev, color = taves_ctry, label = prev_n)) +
  ...
  labs(title = "PREVALENCE: Proportion of participants who endorsed experience",
       subtitle = "By site (rows), question (x-axis), and levels 1-2 grouping of clusters (columns)\nNumbers for each question indicate the total number of participants from that site who responded to that question",
       x = "Question",
       y = "Prevalence (0-1)", color = "Site")
```
...
```{r, fig.width = 6, fig.asp = 0.8}
df_all_resp %>%
  ggplot(aes(x = question, y = app, color = taves_ctry, label = app_n)) +
  facet_grid(cols = vars(level1, level2, cluster), 
             rows = vars(taves_ctry), 
             scales = "free_x", space = "free_x") +
...
```{r, fig.width = 6, fig.asp = 0.8}
df_all_resp %>%
  ggplot(aes(x = question, y = val, color = taves_ctry, label = val_n)) +
  facet_grid(cols = vars(level1, level2, cluster), 
             rows = vars(taves_ctry), 
             scales = "free_x", space = "free_x") +
  geom_hline(yintercept = 0, lty = 2) +
...
```{r, fig.width = 6, fig.asp = 0.8}
df_all_resp %>%
  ggplot(aes(x = question, y = sig, color = taves_ctry, label = sig_n)) +
  facet_grid(cols = vars(level1, level2, cluster), 
             rows = vars(taves_ctry), 
             scales = "free_x", space = "free_x") +
  geom_hline(yintercept = 0.5, lty = 2) +
